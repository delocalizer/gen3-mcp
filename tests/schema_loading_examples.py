...
def test_with_schema_manager():
    ...
    expected_props = [
        "age_at_enrollment", "gender", "race", "ethnicity", 
        "handedness", "experimental_group"
    ]
...
async def test_with_realistic_mcp_setup(mcp_test_setup_realistic):
    ...
    mock_gen3_service = setup["mock_gen3_service"]
    schema = setup["schema"]
    ...
    subject_gender_enum = schema_manager.get_entity_properties("subject")["gender"]["enum"]
...
def test_compare_loading_approaches():
    ...
    from tests.conftest_improved import TEST_SCHEMA
    start_time = pytest.approx(0)  # Instantaneous access
    schema1 = TEST_SCHEMA
